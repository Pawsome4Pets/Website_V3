// Replicates the Pawsome 4 Pets Cognito "New Client Application Form".
// Covers tabs 1, 2, 3, 5, 6 from the original. Tabs 4 (Boarder Request),
// 7 (Grooming T&Cs), and 8 (Booking Details) added later when screenshots arrive.
...
const SLUG = 'new-client-application';
...
const fields = [
  // ════════════════════════════════════════════════════════════════════════════
  // TAB 1 — OWNER'S DETAILS (verbatim, repeating)
  // ════════════════════════════════════════════════════════════════════════════
  { fieldKey: 'sec_owner', type: 'section', label: 'Owner’s Information',
    helpText: 'Information of the primary contact person is required in the below fields.' },

  {
    ...
    options: {
      ...
      fields: [
        ...
        { fieldKey: 'cell',      type: 'tel',   label: 'Cellphone Number',                     isRequired: true,  placeholder: 'Phone (International)' },
        { fieldKey: 'work',      type: 'tel',   label: 'Work / Alternative Contact Number',    isRequired: true,  placeholder: 'Phone (International)' },
        { fieldKey: 'email',     type: 'email', label: 'Email Address',                        isRequired: true },
        { fieldKey: 'email_sec', type: 'email', label: 'Secondary Email Address (Spouse/Partner/Parent etc)', isRequired: true },
        ...
        { fieldKey: 'how_heard', type: 'radio', label: 'How did you hear about us?',           isRequired: true,
  ...
  // ════════════════════════════════════════════════════════════════════════════
  // TAB 2 — EMERGENCY CONTACT PERSON (verbatim, repeating)
  // ════════════════════════════════════════════════════════════════════════════
  { fieldKey: 'sec_emerg', type: 'section', label: 'Emergency Contact Person',
    helpText: 'This information is kept on recored in case we cannot get hold of you. We do not make use of this information other than in case of an emergency.' },

  {
    fieldKey: 'emergency_contacts',
    type: 'repeater',
  ...
  // ════════════════════════════════════════════════════════════════════════════
  // TAB 3 — DOG'S INFORMATION (verbatim, repeating)
  // ════════════════════════════════════════════════════════════════════════════
  { fieldKey: 'sec_dog', type: 'section', label: 'Dog’s Information',
    helpText: 'Tell us about each of your dogs. Click “Add Dog” at the bottom to add another.' },
  ...
  // ════════════════════════════════════════════════════════════════════════════
  // TAB 4 — RAINBOW REQUEST (verbatim from owner)
  // ════════════════════════════════════════════════════════════════════════════
  ...
  // ════════════════════════════════════════════════════════════════════════════
  // TAB 5 — URGENT VETERINARY AUTHORISATION (verbatim from owner)
  // ════════════════════════════════════════════════════════════════════════════
  ...
  // ════════════════════════════════════════════════════════════════════════════
  // TAB 6 — BOARDING TERMS, CONDITIONS AND INDEMNITIES (verbatim from owner)
  // ════════════════════════════════════════════════════════════════════════════
  ...
  // ════════════════════════════════════════════════════════════════════════════
  // TAB 7 — GROOMING TERMS CONDITIONS AND INDEMNITIES (verbatim from owner)
  // ════════════════════════════════════════════════════════════════════════════
  ...
  // ════════════════════════════════════════════════════════════════════════════
  // TAB 8 — BOOKING REQUIREMENTS (verbatim from owner)
  // ════════════════════════════════════════════════════════════════════════════
  ...
  { fieldKey: 'note_thanks', type: 'paragraph',
    helpText: 'Thank you for trusting us to take care of your furry! At Pawsome 4 Pets our guests become family and do we honor the privilage of taking care of your little one in your absense. Our information required is extensive but rest assured that we are acting in the best interest of our fur guests. Your patience during the complesion of the New Client Application Form is appreciated. The document will be retained on file therefore you wont have to complete one on your next booking.' },
];

// Auto-label any paragraph that has no label but starts with a clause number
// (e.g. "1.1.", "2.2.1") so the admin builder list is readable.
fields.forEach((f) => {
...
async function main() {
  ...
  if (!form) {
    const created = await (await fetch(`${API}/api/admin/forms`, {
      ...
      body: JSON.stringify({
        title: 'Pawsome 4 Pets Dog Hotel & Spa - New Client Application Form',
        slug: SLUG,
      }),
    })).json();
    form = created.form;
  }
  console.log(`Form id: ${form.id}, slug: ${form.slug}, ${fields.length} fields incoming`);
  ...
  await (await fetch(`${API}/api/admin/forms/${form.id}`, {
    ...
    body: JSON.stringify({
      ...
      isPublished: true,
      createsAccount: true,
      successMessage: 'Thank you! We’ve received your application and will follow up shortly. We will get in touch via the email and contact number provided.',
    }),
  })).json();

  console.log('\nView:  http://localhost:5173/forms/' + form.slug);
  console.log('Edit:  http://localhost:5173/admin/forms/' + form.id);
}
